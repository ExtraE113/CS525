...
: term):
term =
TMapp(TMllist_next_raw, t)
where {
val TMllist_next_raw =
  TMlam("input",
        TMeval(TMllist_tail(TMvar
"input"))
)
}

fun TMllist_enumerate(t
...
var input = TManno(TMvar
"input",
TPllist(input_type)
)
var cur_index = TManno(TMvar
"cur_index", TPint)
...
TMlazy (TMapp(TMapp(TMvar

"cont",
TMadd(TMvar
"from", TMint(1))), TMvar"to"))
)
)
)
)
...
val
zero_to_five = TMapp(TMapp(TMlazy_range, TMint(0)), TMint(5))
//
...
fun TMllist_get_at(t

:term, ind: term) :
term =
TMapp(TMapp(TMllist_get_at_raw, t), ind)
where {
...
:term, ind: term) :
term =
TMapp(TMapp(TMtakeout_raw, t), ind)
where {
var TMtakeout_raw =
  let
var element_type = tpxyz_new()
...
: term, func: term):
term =
TMapp(TMapp(TMlazy_map_raw, input), func)
where {
val
  TMlazy_map_raw =
  let
...
val input = TManno(TMvar
"input",
TPllist(before_type)
)
val func = TManno(TMvar
"func",
TPfun(before_type, after_type
))
...
val TMtakeouts =
  TMlam(
    "input",
    TMlazy_map(
      TMapp(TMrange_len, TMvar
"input"),

TMapp(TMtakeout_curried(), TMvar
...
val input1 = TManno(TMvar
"input1",
TPllist(element_type)
)
val input2 = TManno(TMvar
"input2",
TPllist(element_type)
)
...
fun TMlazy_flatten(t

:term):
term =
TMapp(TMlazy_flatten_raw, t)
where {
...
TPllist (TPtup(element_type, input_type))
...
"elem_remainder_tuple"), input_type)),
TPllist(input_type)
),
TMlazy_map(
  TMvar
...
TManno (TMllist_cons(TMfst(TMvar
...
val
one_to_three_inclusive = TMapp(TMapp(TMlazy_range, TMint(1)), TMint(4))
  (*
     val() = println
!(
...
term_eval0(TMfst(
  TMllist_next(TMllist_next(TMllist_next(TMllist_next(TMllist_next(TMlazy_map(TMapp(TMordered_permute,
  zero_to_five), TMllist_to_list
